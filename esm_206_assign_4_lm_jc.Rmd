...
Lobster sizes in the three Non-Marine Protected Areas were observed to have less than noticeable shifts in lobster sizes whereas lobsters. While lobster sizes in Marine Protected Areas did have noticeable shifts. 
...
```{r, fig.cap="Figure 5: Lobster size in Marine Protectd Areas and Non-Marine Protected Areas in 2012 and 2018 kernal denisty for mean lobster size in Marine Protected Areas in 2012 (dark blue) and 2018 (light blue) and Non-Marine Protected Areas in 2012 (dark orange) and 2018 (light orange). The means are shown as dashed lines. Data: SBC LTER}
...
For the two Marine protected areas, comparing the means between 2012 and 2018 (n = `r n_mpa`), the mean for 2012 is `r round(mean_mpa_2012, 2)` $\pm$ `r round(sd_mpa_2012, 2)`, and the mean for 2018 is `r round(mean_mpa_2018, 2)` $\pm$ `r round(sd_mpa_2018, 2)` with an actual difference in mean size between the two years being `r round(mean_mpa_2018 - mean_mpa_2012, 2)` (two-sided t-test, t(`r round(mpa_2012_2018_ttest$parameter, 2)`) = `r round(mpa_2012_2018_ttest$statistic, 2)`, p = `r round(mpa_2012_2018_ttest$p.value, 4)`)

For the three Non-Marine Protected Areas, comparing the means between 2012 and 2018, the mean for 2012 is `r round(mean_non_mpa_2012, 2)` $\pm$ `r round(sd_non_mpa_2012, 2)`, the mean for 2018 is `r round(mean_non_mpa_2018, 8)` $\pm$ `r round(sd_non_mpa_2012, 2)` with an actual difference in mean size between the two years being `r round(mean_non_mpa_2018 - mean_non_mpa_2012, 2)` (two-sited t-test, t(`r round(non_mpa_2012_2018_ttest$parameter, 2)`) = `r round(non_mpa_2012_2018_ttest$statistic, 2)`, p = `r round(non_mpa_2012_2018_ttest$p.value, 2)`)
